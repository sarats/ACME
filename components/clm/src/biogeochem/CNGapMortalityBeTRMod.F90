module CNGapMortalityBeTRMod
  ...
  public :: readCNGapMortBeTRParams
...
contains
  ...
  !-----------------------------------------------------------------------
  subroutine readCNGapMortBeTRParams ( ncid )
    !
    ! !DESCRIPTION:
    ! Read in parameters
    !
    ...
    call set_gap_indicators
  end subroutine readCNGapMortBeTRParams
  ...
  subroutine CNGapMortality (&
    ...
    associate(                                                                                              &
      ...
      do fp = 1,num_soilp
         ...
         if (spinup_state >= 1) then
           nitrogenflux_vars%m_deadstemn_to_litter_patch(p)         = nitrogenflux_vars%m_deadstemn_to_litter_patch(p) * spinup_mortality_factor
           nitrogenflux_vars%m_deadcrootn_to_litter_patch(p)        = nitrogenflux_vars%m_deadcrootn_to_litter_patch(p) * spinup_mortality_factor
         end if
            ...
            do fc = 1,num_soilc
               ...
               if (pi <=  col%npfts(c)) then
                  ...
                  if (pft%active(p)) then
                     write(*,*)'===================================='
                     write(*,*)'01 m_leafn_to_litter             =',p,m_leafn_to_litter(p)
                     write(*,*)'02 m_frootn_to_litter            =',p,m_frootn_to_litter(p)
                     write(*,*)'03 m_retransn_to_litter          =',p,m_retransn_to_litter(p)
                     write(*,*)'04 m_leafn_storage_to_litter     =',p,m_leafn_storage_to_litter(p)
                     write(*,*)'05 m_frootn_storage_to_litter    =',p,m_frootn_storage_to_litter(p)
                     write(*,*)'06 m_livestemn_storage_to_litter =',p,m_livestemn_storage_to_litter(p)
                     write(*,*)'07 m_deadstemn_storage_to_litter =',p,m_deadstemn_storage_to_litter(p)
                     write(*,*)'08 m_livecrootn_storage_to_litter=',p,m_livecrootn_storage_to_litter(p)
                     write(*,*)'09 m_deadcrootn_storage_to_litter=',p,m_deadcrootn_storage_to_litter(p)
                     write(*,*)'10 m_leafn_xfer_to_litter        =',p,m_leafn_xfer_to_litter(p)
                     write(*,*)'11 m_frootn_xfer_to_litter       =',p,m_frootn_xfer_to_litter(p)
                     !write(*,*)'12 m_livestemn_xfer_to_litter    =',p,m_livestemn_xfer_to_litter(p)
                     !write(*,*)'13 m_deadstemn_xfer_to_litter    =',p,m_deadstemn_xfer_to_litter(p)
                     !write(*,*)'14 m_livecrootn_xfer_to_litter   =',p,m_livecrootn_xfer_to_litter(p)
                     !write(*,*)'15 m_deadcrootn_xfer_to_litter   =',p,m_deadcrootn_xfer_to_litter(p)
                endif
              endif
            enddo
         enddo
...
end module CNGapMortalityBeTRMod
